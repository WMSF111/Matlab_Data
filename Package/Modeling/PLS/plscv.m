%此代码主要用于实现PLS模型的交叉验证，选择最佳的潜在变量数，并通过计算误差指标（如RMSECV、Q^2等）来评估模型的性能。
...
%            y: m x 1  (measured property) (测量属性)
...
%       方法：预处理法。 包含：自动缩放, 中心 最大最小，无
%               
%
%      PROCESS: =1 : print process.       打印过程。
%               =0 : don't print process. 不打印过程。
%+++ Order: =0  sorted, default. For CV partition. 0 排序，默认值。用于 CV 分区。
%           =1  random. 随机
%           =2  original. 原始
%+++ Output: Structural data: CV  输出：结构数据：CV
