function result = plot_e_nose_time_series(temp_c, feature_modes, data_root, lang_mode, normalize_mode, use_baseline_removed)
% Plot E-nose feature time series for one temperature with interactive controls.
...
if nargin < 6 || isempty(use_baseline_removed)
    use_baseline_removed = false;
end

S = load_e_nose_time_data(temp_c, feature_modes, data_root, use_baseline_removed);
labels = local_get_labels(lang_mode);
nvar = numel(S.feature_labels);
ncols = min(4, max(1, ceil(sqrt(nvar))));
nrows = ceil(nvar / ncols);
group_list = S.group_list(:)';
group_safe_list = S.group_safe_list(:)';
batch_colors = lines(max(3, numel(group_list)));
avg_color = [0 0 0];

fig = figure('Name', sprintf('E_nose_%dC', temp_c), ...
    'NumberTitle', 'off', ...
    'Color', 'w');
tiledlayout(fig, nrows, ncols, 'Padding', 'compact', 'TileSpacing', 'compact');

plot_data = repmat(local_empty_plot_meta(), nvar, 1);
result_table = table(S.minute_grid(:), 'VariableNames', {'time_min'});

for i = 1:nvar
    ax = nexttile;
    hold(ax, 'on');
    raw_all_handles = gobjects(0);
    raw_avg_handle = gobjects(1);
    slope_all_handles = gobjects(0);
    slope_avg_handle = gobjects(1);
    raw_all_items = {};
    raw_avg_items = {};
    slope_all_items = {};
    slope_avg_items = {};
    ...
    for bi = 1:numel(group_list)
        ...
        h_raw = plot(ax, x_obs, y_obs_norm, '-o', ...
            'Color', batch_colors(bi, :), ...
            'LineWidth', 1.0, ...
            'MarkerSize', 4, ...
            'DisplayName', local_group_label(labels, group_name));
        raw_all_handles(end+1) = h_raw; %#ok<AGROW>
        raw_all_items{end+1} = local_group_label(labels, group_name); %#ok<AGROW>
        ...
        h_slope = plot(ax, sx, sy, '--', ...
            ...
            'LineWidth', 1.0, ...
            'Visible', 'off', ...
            'DisplayName', local_group_slope_label(labels, group_name));
        slope_all_handles(end+1) = h_slope; %#ok<AGROW>
        slope_all_items{end+1} = local_group_slope_label(labels, group_name); %#ok<AGROW>

        result_table.(sprintf('%s_%s', matlab.lang.makeValidName(S.feature_labels{i}), matlab.lang.makeValidName(group_safe))) = ...
    ...
    [mean_norm, norm_info] = normalize_plot_series(mean_raw(valid_mean), normalize_mode, mean_raw(valid_mean));
    raw_avg_handle = plot(ax, S.minute_grid(valid_mean), mean_norm, '-s', ...
        'Color', avg_color, ...
        'LineWidth', 1.8, ...
        'MarkerSize', 5, ...
        'Visible', 'off', ...
        'DisplayName', labels.avg_legend);
    raw_avg_items = {labels.avg_legend};

    [sx_avg, sy_avg] = local_compute_slope(S.minute_grid(valid_mean), mean_raw(valid_mean));
    slope_avg_handle = plot(ax, sx_avg, sy_avg, '-', ...
        'Color', avg_color, ...
        'LineWidth', 2.0, ...
        'Visible', 'off', ...
        'DisplayName', labels.slope_avg_legend);
    slope_avg_items = {labels.slope_avg_legend};
    ...
    title(ax, sprintf(labels.title_fmt, temp_c, S.feature_labels{i}, raw_min, raw_max), 'Interpreter', 'none');
    xlabel(ax, labels.x_label);
    ylabel(ax, sprintf('%s%s', S.feature_labels{i}, local_get_y_suffix(norm_info, lang_mode)), 'Interpreter', 'none');
    grid(ax, 'on');
    xlim(ax, [min(S.minute_grid) max(S.minute_grid)]);

    plot_data(i).ax = ax;
    plot_data(i).raw_all_handles = raw_all_handles;
    plot_data(i).raw_avg_handles = raw_avg_handle;
    plot_data(i).slope_all_handles = slope_all_handles;
    plot_data(i).slope_avg_handles = slope_avg_handle;
    ...
    result_table.(sprintf('%s_mean', matlab.lang.makeValidName(S.feature_labels{i}))) = mean_raw;
end

setappdata(fig, 'e_nose_plot_data', plot_data);
setappdata(fig, 'e_nose_labels', labels);
setappdata(fig, 'e_nose_lang_mode', lang_mode);
local_add_controls(fig, labels);
local_apply_modes(fig);

project_root = fileparts(fileparts(fileparts(mfilename('fullpath'))));
out_dir = fullfile(project_root, 'Result', 'Summary', sprintf('E_nose_%dC', temp_c));
if ~exist(out_dir, 'dir')
    mkdir(out_dir);
end
writetable(result_table, fullfile(out_dir, sprintf('%dC_e_nose_features.csv', temp_c)));
saveas(fig, fullfile(out_dir, sprintf('%dC_e_nose_features.tif', temp_c)));
...
if is_zh
    ...
    labels.slope_avg_legend = char([24179 22343 20540 26012 29575]);
    labels.x_label = char([26102 38388 47 109 105 110]);
    labels.title_fmt = '%d摄氏度 | %s | 最小值=%.4f 最大值=%.4f';
    labels.group_fmt = '%s';
    labels.group_slope_fmt = '%s斜率';
    labels.y_suffix_raw = char([65288 26410 24402 19968 21270 65289]);
else
    ...
    labels.title_fmt = '%dC | %s | Min=%.4f Max=%.4f';
    labels.group_fmt = '%s';
    labels.group_slope_fmt = '%s Slope';
    labels.y_suffix_raw = ' (Raw)';
end
end
...
function txt = local_group_label(labels, group_name)
txt = sprintf(labels.group_fmt, group_name);
end

function txt = local_group_slope_label(labels, group_name)
txt = sprintf(labels.group_slope_fmt, group_name);
end
