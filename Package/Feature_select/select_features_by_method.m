...
%   1) pca
%      - 无监督方法，只利用 X 的整体方差结构，不直接使用 y
%      - 当前这个 pca 分支实现的是“PCA 波段筛选”
%      - 它不是“PCA 主成分投影”
%      - 也就是说，这里最后返回的仍然是 selected_idx，
%        即若干原始波段索引，而不是 PC1、PC2 这类主成分得分
%      - 它的流程是：
%        先做 PCA -> 再根据主成分 loading/贡献给原始波段打分 ->
%        最后从原始波段里选 top_k 个
%      - 如果要做“PCA 主成分投影”，那是另一类特征表达方式：
%        不再保留原始波段索引，而是直接把 X 投影到主成分空间
%      - 当前项目里，“PCA 主成分投影”主要在训练阶段由
%        train_model_with_generalization.m 的 local_apply_pca_projection 实现
%   2) corr_topk
%      - 监督式训练前筛选
...
switch method
    ...
    case 'pca'
        % 这里实现的是“PCA 波段筛选”：
        % 输出仍然是原始波段索引 selected_idx，而不是投影后的主成分特征。
        ...
        fs_result.info.pca_mode = 'band_selection';
