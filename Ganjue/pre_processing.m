...
% 平滑数据保存文件
smooth_mat_file_name = sprintf('%s\\Data processing\\Result\\Smooth\\ganjue\\Smooth_Results.mat', ...
                       folder);
smooth_file_name = sprintf('%s\\Data processing\\Result\\Smooth\\ganjue\\Smooth_Results.tif', ...
                   folder);  
%% 加载数据
...
%%结果较为稳定
% SG_data = SG(data,3,7);
% MSC_data = Standardization(SG_data);
% SNV_data = MSC_data;

%%表现还可以
% SG_data = SG(data,3,7);
% MSC_data = normalization(SG_data,1);
% SNV_data = MSC_data;

%%还可以
% SG_data = SG(data,3,7);
% MSC_data = normalization(SG_data,2);
% SNV_data = SNV(MSC_data);

%% 还行
% SG_data = normalization(data,2);
% MSC_data =  SG(SG_data,3,7);
% SNV_data = MSC(MSC_data);

%% 好结果
SG_data = Standardization(data);
MSC_data = SG(SG_data,3,13);
SNV_data = MSC(MSC_data);


% SG_data = SG(data,3,7); %SG滤波
