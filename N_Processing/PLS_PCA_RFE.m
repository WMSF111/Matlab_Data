...
Result_folder = sprintf('%s\\Data processing\\Result\\Model\\N\\PLS_PCA_RFE',folder);% 结果保存文件夹
...
Xtrain_PCA_POST =  Xtrain  * coeff(:, 1:optimal_result.PCAK+10);
% 使用相同的主成分变换应用于测试集
Xtest_PCA_POST = Xtest  * coeff(:, 1:optimal_result.PCAK+10);
% 执行递归特征消除，保留2个重要特征
% 参数说明：
% 'cv' - 10折交叉验证
% 'direction' - 后向消除
[selectedFeatures, history] = sequentialfs(fun, Xtrain_PCA_POST, Ytrain, ...
                              'cv', 10, ...
                              'direction', 'backward', ...
                              'options', opts);
RFE_wave_select = find(selectedFeatures);
Xtrain_RFE = Xtrain_PCA_POST(:,RFE_wave_select);  
Xtest_RFE = Xtest_PCA_POST(:,RFE_wave_select); 
