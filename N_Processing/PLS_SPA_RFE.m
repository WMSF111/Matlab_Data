...
physical_file_name = 'yingdu.xlsx';  % 理化值文件名
Smooth_folder = sprintf('%s\\Data processing\\Result\\Smooth\\N\\Smooth_Results.mat',folder);% 光谱波形数据文件
Psyhical_data_folder = sprintf('%s\\Data processing\\data\\physical\\%s',folder,physical_file_name); % 样品理化值数据文件
Result_folder = sprintf('%s\\Data processing\\Result\\Model\\N\\PLS_RFE',folder);% 结果保存文件夹
...
for s = 1 : size(Xtrain,2)
   for k = 1 : size(Xtrain,2)
        ...
        Ypred = plsval(PLS,Xtrain_SPA,Ytrain);
        % 性能评估        
        SST_train = sum((Ytrain - mean(Ytrain)).^2);
        SSE_train = sum((Ytrain - Ypred).^2);
...
wave_select_post = SPA(Xtrain,optimal_result.SPAS,optimal_result.SPAK+10);
Xtrain_SPA_post = Xtrain(:,wave_select_post);  
Xtest_SPA_post = Xtest(:,wave_select_post);
% 执行递归特征消除，保留2个重要特征
% 参数说明：
% 'cv' - 10折交叉验证
% 'direction' - 后向消除
[selectedFeatures, history] = sequentialfs(fun, Xtrain_SPA_post, Ytrain, ...
                              'cv', 10, ...
                              'direction', 'backward', ...
                              'options', opts);
RFE_wave_select = find(selectedFeatures);
Xtrain_RFE = Xtrain_SPA_post(:,RFE_wave_select);  
Xtest_RFE = Xtest_SPA_post(:,RFE_wave_select); 
...
RPD = std(Ytest) / RMSEP;
optimal_result.A = A;
optimal_result.SPA_wave_select = wave_select_post;
optimal_result.RFE_wave_select = RFE_wave_select;
...
fprintf("SPA波长选择：%d,RFE波长选择：%d\r\n",optimal_result.SPA_wave_select,optimal_result.RFE_wave_select)
